...
@Component
@Slf4j
public class BlueConsumer implements ConsumerSeekAware {


    private final DataService dataService;

    public BlueConsumer(final DataService dataService) {
        this.dataService = dataService;
    }


    @KafkaListener(topics = "blue", containerFactory = "kafkaListenerContainerFactory")
    public void receive(ConsumerRecord<String, Record> record, Acknowledgment acknowledgment) {
        ...
        if (record.value().getLocation() == null
            ...
            dataService.addBlue(record.value());
        }
